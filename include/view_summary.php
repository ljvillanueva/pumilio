...
ob_flush(); flush();
	...
	for ($i=0;$i<$nrows;$i++) {
		...
		echo "<dl class=\"dl-horizontal\">";
			...
			if (isset($Date) && $Date!="")
				echo "<dt>Date</dt><dd>$Date_h</dd><dt>Time</dt><dd>$Time</dd>";
			...
			$QualityFlag = DB::column('SELECT QualityFlag FROM `QualityFlags` WHERE QualityFlagID = ' . $QualityFlagID);
				...
				echo "<dt>File Quality flag</dt><dd><em title=\"$QualityFlag\">$QualityFlagID</em></dd>";
				if ($DerivedSound == "1"){
					echo "<dt>Derived from</dt><dd><a href=\"db_filedetails.php?SoundID=$DerivedFromSoundID\">$DerivedFromSoundID</dd>";
					}
