...
$no_weather=query_one("SELECT COUNT(*) from WeatherSites", $connection);
	...
	if ($no_weather==0) {
		...
		echo "<select name=\"WeatherSiteID\" class=\"fg-button ui-state-default ui-corner-all\" style=\"font-size:12px\">";
		...
		echo "<form action=\"delete_weatherdata.php\" method=\"POST\" id=\"del_weatherdata\" name=\"del_weatherdata\">";
			$query_w = "SELECT * from WeatherSites ORDER BY WeatherSiteName";
			$result_w = mysqli_query($connection, $query_w)
				or die (mysqli_error($connection));
			$nrows_w = mysqli_num_rows($result_w);
...
echo "<p>Other tasks:
	...
	<input type=submit value=\" Add weather sites \" class=\"fg-button ui-state-default ui-corner-all\" style=\"font-size:12px\"></form>";
	
echo "<br>&nbsp;&nbsp;&nbsp;<form action=\"browse_weather.php\" method=\"GET\">
	<input type=submit value=\" View weather sites in a map \" class=\"fg-button ui-state-default ui-corner-all\" style=\"font-size:12px\"></form>";
